...
-- Run log for the insights page. Additive; safe to re-run on an existing database.
...
-- Insights. Rows persist across regenerations so week-over-week deltas
-- survive. Additive; safe to re-run.
...
-- Insights predating these columns inherit firstSeenAt = lastSeenAt from the
-- defaults, which the UI reads as "new in the latest run". Date them from when
-- the row was created instead. Rows the current generator writes always have
-- signals, so they are untouched.
...
CREATE INDEX IF NOT EXISTS "insights_org_archived_idx" ON "public"."insights" USING "btree" ("orgId", "isArchived");

CREATE INDEX IF NOT EXISTS "insight_reports_org_created_idx" ON "public"."insight_reports" USING "btree" ("orgId", "createdAt" DESC);

CREATE INDEX IF NOT EXISTS "user_upvote_content_id_idx" ON "public"."user_upvote" USING "btree" ("contentId");
