  ...
  try {
    while (retries > 0) {
      ...
      const systemPrompt = `
        ...
        [
          {
            ...
            "category": "string",   // "idea", "issue", "general feedback"
        ...
        - **Group Related Issues:** Combine related issues into a single, well-defined fix. For instance, reports of “application freezes on load,” “data fetch timeouts,” and “slow component rendering” should be grouped into an actionable item like “Optimize Widget Loading Performance.”
        ...
        - **Prioritization Formula:** Your prioritization should be heavily weighted by the following factors, in order:
          1.  **Severity & Urgency:** Critical issues and usability blockers that prevent users from completing core tasks must be given the highest weight.
          ...
          3.  **Category Boost:** Apply a significant boost to items categorized as 'issue' over 'idea' or 'general feedback'.
        ...
        \`\`\`json
        {
          ...
          "items": {
            ...
            "properties": {
              ...
              "category": {
                "type": ["string", "null"],
                "enum": ["idea", "issue", "general feedback", null]
              },
              "ids": {
                "type": "array",
                "items": {
